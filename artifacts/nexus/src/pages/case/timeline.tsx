...
import { TimelineEvent, Citation } from '@/data/mock-case';
...
export default function CaseTimeline() {
  ...
  const datedEvents = timeline.filter(e => e.dateType !== 'unknown').sort(
    (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()
  );
  const undatedEvents = timeline.filter(e => e.dateType === 'unknown');
  ...
  const hiddenCount = timeline.length - (filteredDated.length + filteredUndated.length);
  const conflictCount = timeline.filter(e => e.dateConflict).length;
  ...
  const FILTERS: { key: FilterKey; label: string; count?: number }[] = [
    { key: 'all',         label: 'All Events',     count: timeline.length },
      ...
      <div className="border-b border-border bg-card/80 px-5 py-3 flex items-center justify-between shrink-0">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5 font-mono">
            {timeline.length} source-linked events · Approximate dates are marked — they are not exact
          </p>
        </div>
