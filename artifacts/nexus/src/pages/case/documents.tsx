...
import { Document } from '@/data/mock-case';
...
export default function CaseDocuments() {
  ...
  const totalSegments = documents.reduce((s, d) => s + (d.canonicalSegments || 0), 0);
  const totalImageOnly = documents.reduce((s, d) => s + d.pages.filter(p => p.status === 'image-only').length, 0);
  const totalFailed = documents.reduce((s, d) => s + d.pages.filter(p => p.status === 'extraction-failed' || p.status === 'missing').length, 0);
