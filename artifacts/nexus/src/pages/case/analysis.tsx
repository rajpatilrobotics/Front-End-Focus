...
import { Finding, ContextGap, Citation, ReviewLane } from '@/data/mock-case';
...
export default function CaseAnalysis() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(
    findings.filter(f => f.lane === 'A')[0]?.id || null
  );
  ...
  const executeWithdraw = (id: string) => {
    dispatch({ type: 'WITHDRAW_EVIDENCE', id });
    setShowCascadeModal(false);
    setWithdrawingId(null);
  };

  const handleAction = (id: string, action: Finding['reviewStatus']) => {
    dispatch({ type: 'UPDATE_FINDING', id, reviewStatus: action });
  };

  const handleGapAction = (id: string, status: ContextGap['status']) => {
    dispatch({ type: 'UPDATE_CONTEXT_GAP', id, status });
  };
