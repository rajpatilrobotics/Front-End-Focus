...
export default function CaseLayout() {
  ...
  // Shared state
  const { state, selectors, dispatch } = useCaseContext();
  const caseData = state.cases.find(c => c.id === id) || state.cases[0];
  ...
  // Derive live badge counts from shared state via selectors
  const { pendingFindingsCount, immediateNeedsCount, openGapsCount, pendingQuestionsCount, openTasksCount, isExportBlocked } = selectors;
