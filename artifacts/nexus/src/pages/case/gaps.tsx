...
import { EvidenceGap, GapStatus } from '@/data/mock-case';
...
export default function CaseGaps() {
  ...
  const setStatus = (id: string, status: GapStatus) => {
    dispatch({ type: 'UPDATE_EVIDENCE_GAP', id, patch: { status } });
  };
  ...
  const confirmResolve = () => {
    ...
    const gap = gaps.find(g => g.id === resolveModal.gapId);
    if (!gap) return;
    dispatch({
      type: 'UPDATE_EVIDENCE_GAP',
      id: resolveModal.gapId,
      patch: {
